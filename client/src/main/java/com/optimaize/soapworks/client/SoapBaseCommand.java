...
public abstract class SoapBaseCommand<T, A, R> extends BaseCommand<A, R> {
    ...
    @NotNull
    protected PortUrlFactory getPortUrlFactory(@NotNull ExecutionContext ec) {
        return ec.getMode().get(Keys.PORT_URL_FACTORY).get();
    }

    @NotNull
    protected URL makeUrl(@NotNull ExecutionContext ec, @NotNull String servicePath) {
        Host host = getHost(ec);
        PortUrlFactory urlFactory = getPortUrlFactory(ec);
